...
MissionControl::MissionControl() : CommonNode("mission_control") {
    ...
    node_map["/node_1"].can_start_mission =
    ...
    control_subscription =
        this->create_subscription<interfaces::msg::FlyToCoord>(
            "fly_to_coord", 10,
 ...
 */
void MissionControl::set_standby_config() { active_node = ""; }

/**
 * @brief Sets the mission state to a new value.
 *
 * This function sets the mission state to the specified new value. If the new
 * value is different from the current mission state, the `state_first_loop`
...
void MissionControl::set_mission_state(const MissionState_t new_mission_state) {
    if (mission_state != new_mission_state) state_first_loop = true;
...
void MissionControl::mode_self_check() {
    ...
    if (i >= max_wait_time) {
        RCLCPP_ERROR(this->get_logger(),
                     "MissionControl::mode_self_check: Self check failed: Not "
                     "all heartbeats received in the given timeframe (%us)",
                     (max_wait_time * event_loop_time_delta_ms) / 1000);
        mission_abort(
...
void MissionControl::initiate_takeoff() {
    ...
    RCLCPP_INFO(this->get_logger(),
                "MissionControl::initiate_takeoff: Waypoint state activated");
}
...
void MissionControl::mission_abort(std::string reason) {
    ...
    // Stop node
    RCLCPP_INFO(this->get_logger(),
                "MissionControl::mission_abort: Mission abort message sent, "
                "exiting now");
    exit(EXIT_FAILURE);
}
...
void MissionControl::heartbeat_callback(const interfaces::msg::Heartbeat &msg) {
    ...
    if (msg.sender_id == get_fully_qualified_name()) return;

    if (node_map.find(msg.sender_id) == node_map.end()) {
        RCLCPP_ERROR(get_logger(),
                     "MissionControl::heartbeat_callback: Received "
                     "unregistered heartbeat: %s",
                     msg.sender_id.c_str());
        return;
    }
    ...
    if (msg.active && (msg.sender_id != active_node)) {
        RCLCPP_FATAL(get_logger(),
                     "MissionControl::heartbeat_callback: Node '%s' is active "
                     "even though it should be deactive",
                     msg.sender_id.c_str());
        mission_abort("A node was active even though it should be deactive");
    }
    if ((!msg.active) && (msg.sender_id == active_node)) {
        RCLCPP_FATAL(get_logger(),
                     "MissionControl::heartbeat_callback: Node '%s' is "
                     "deactive even though it should be active",
                     msg.sender_id.c_str());
...
void MissionControl::heartbeat_timer_callback() {
    ...
    for (auto &nm : node_map) {
        ...
        if (hp.received == false) {
            err_flag = true;
            RCLCPP_FATAL(this->get_logger(),
                         "MissionControl::heartbeat_timer_callback: No "
                         "heartbeat from '%s' received!",
                         nm.first.c_str());
        }
