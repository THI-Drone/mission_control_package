...
MissionControl::MissionControl() : CommonNode("mission_control")
{
    ...
    control_subscription = this->create_subscription<interfaces::msg::Control>(
        "control", 10, std::bind(&MissionControl::check_control, this, _1));
...
// Fail-Safe checks
void MissionControl::check_control(const interfaces::msg::Control &msg)
{
    // TODO add control check implementation to check that only an active node sends on the control topic
}
...
void MissionControl::heartbeat_callback(const interfaces::msg::Heartbeat &msg)
{
    ...
    if (msg.sender_id == this->get_fully_qualified_name())
        return;
